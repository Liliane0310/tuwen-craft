...
- 复古 Field Guide 风
- 线圈方格笔记风
- 社交风（头像 / ID / 关注 / 听 / 搜索 / 更多）

「社交风」的正文摘要提供 H1、H2 和独立高亮工具：标题固定使用汇文明朝体，高亮只改变文字背景，不改变文字颜色。

支持在封面上添加标注（annotation），导出时一并渲染。

### 正文编辑（卡片模式）

把 Markdown 正文自动分页成多张 864×1152 的卡片，右侧按顺序预览，可选择单张导出，也可一键批量导出全部 PNG。
...
点工具栏「图片」按钮上传本地图片，也可以把剪贴板里的图片直接粘贴到正文。图片会自动以 `[[image:编号]]` 标记插入；显示大小写成 `[[image:编号|50%]]`。上传后可在弹窗中裁剪和缩放，之后点击预览卡片里的正文图片也能再次调整。图片以 base64 存在本地，刷新不丢。
...
字号、行距、文字色、背景色、中英文字体（苹方 / 宋体 / 楷体 / 黑体 / 霞鹜文楷 / 汇文明朝体 / 系统 Serif / Rounded / Mono / Lora 等）。

正文卡片可切换为「社交风」：每页重复显示可编辑头像与 ID，以及关注、听、搜索、更多动作；一级标题固定使用汇文明朝体，适合连续发布一张一张的系列图文。
